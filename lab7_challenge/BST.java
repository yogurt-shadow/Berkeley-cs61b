...
public class BST<Key extends Comparable<Key>> {
    ...
    public double aver_depth(){
        return (double) sum_depth() / (double) size();
    }

    private int sum_depth(){
       return sum_depth(root);
    }

    private int sum_depth(Node x){
        if(x == null || x.size == 1){return 0;}
        if(x.left == null){
            return sum_depth(x.right) + x.right.size;
        }
        if(x.right == null){
            return sum_depth(x.left) + x.left.size;
        }
        return sum_depth(x.left) + x.left.size + sum_depth(x.right) + x.right.size;
    }
    ...
    public static void main(String[] args){
        ...
        System.out.println(a.sum_depth());
        System.out.println(a.aver_depth());
    }
}
