...
public class MergeSort {
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
            Queue<Item> items) {
        // Your code here!
        Queue<Queue<Item>> current = makeSingleItemQueues(items);
        while (current.size() > 1) {
            Queue<Item> first = current.dequeue();
            Queue<Item> second = current.dequeue();
            Queue<Item> merge = mergeSortedQueues(first, second);
        }
